...
const App: React.FC = () => {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'free':
        return <FreeResources onNavigate={setCurrentPage} />;
      case 'paid':
        return <PaidResources onAddToCart={addToCart} onNavigate={setCurrentPage} />;
      case 'about':
  ...
  const shouldShowFloatingCart = cart.length > 0 && currentPage === 'paid';

  return (
    <div className="flex flex-col min-h-screen selection:bg-primary selection:text-white bg-white dark:bg-black transition-colors duration-300">
      <Navbar 
