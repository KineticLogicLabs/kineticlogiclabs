...
const Navbar: React.FC<NavbarProps> = ({ currentPage, onNavigate, cartCount, showCart, isDarkMode, toggleTheme }) => {
  ...
  const navLinks: { label: string; value: Page }[] = [
    ...
    { label: 'Premium Assets', value: 'paid' },
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] glass-nav border-b border-black/5 dark:border-white/5 bg-white/70 dark:bg-black/75 px-6 py-5 md:py-6">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        >
          <Logo className="h-10 md:h-14 w-auto transition-colors duration-300 group-hover:text-primary" />
        </div>
        ...
        <div className="hidden md:flex items-center gap-8 lg:gap-12">
          {navLinks.map((link) => (
            <button
              ...
              className={`text-sm md:text-base font-semibold tracking-wide transition-colors duration-200 ${
          ...
          <div className="flex flex-col p-10 gap-10">
            {navLinks.map((link) => (
              <button
                ...
                className={`text-xl font-semibold text-left transition-colors duration-200 ${
